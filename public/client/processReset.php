<?php 
    ...
    if(empty($uid)) die(header("Location: /reset-password/?nr=1"));

    if(isset($_POST['reset_password']))
    {
        if($_POST['password'] != $_POST['confirm_password'])
            die(header("Location: /reset/$resetid/?pe=1"));

        $user = new User($uid);
        $user->password = password_hash($_POST['password'], PASSWORD_DEFAULT);
        update_user($uid, $user);
        delete_reset_request($resetid);
        die(header("Location: /account/login?ps=1"));
    }
				...
				<div class="row">
					<div class="col-md-12 col-lg-12">
						<div class="title-single-box">
							<h1 class="title-single">Reset Your Password</h1>
						</div>
					</div>
                    <div class="col-lg-6 py-5 mx-auto">
                        <div class="card p-2 shadow rounded-0">
                            <div class="card-body">
                                <form action="/reset/<?php echo $resetid ?>" method="POST">
                                    <?php
                                        if(isset($_GET["pe"])) {
                                            echo '
